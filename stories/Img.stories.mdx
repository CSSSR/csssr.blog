...
<Meta
  ...
  argTypes={{
    ...
    imageName: {
      type: 'radio',
      options: ['manWithCafeBg','mySampleImageSmall','peopleWithBasket','peopleWithDogBg','peopleWithGramophone'],
    }
  }}
/>

export const Img = (args) => {
  const attributes = Object.keys(args).reduce((acc, item) => {
    acc += `${item}="${args[item]}" `;
    return acc;
  }, '');
  return (
