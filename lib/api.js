...
import { getPictureData } from '@csssr/csssr.images/dist/utils'
const postsDirectory = join(process.cwd(), '_posts')

const requireAllPostsImages = require.context('../public/assets/blog/posts', true)
...
const allPostsImages = requireAllPostsImages.keys().reduce((acc, key) => {
  ...
  if (process.env.INCLUDE_EXAMPLE_POST || slug !== 'example') {
    if (!acc[slug]) {
      acc[slug] = {}
    }
...
export function getAllPosts(fields = []) {
  ...
  return slugs
    .filter((slug) => process.env.INCLUDE_EXAMPLE_POST || slug !== 'example.md')
