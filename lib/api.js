...
import dataByLanguagePostAndImage from './allPostsImages'
...
export async function getPostBySlugAndLanguage(slug, language, fields = []) {
  ...
  fields.forEach((field) => {
    ...
    if (field === 'images') {
      items[field] =
        dataByLanguagePostAndImage[language][slug] ||
        dataByLanguagePostAndImage['any-language']['any-article']
    }
    if (data[field]) {
...
export async function getPostNewsBySlug(slug, language, fields = []) {
  ...
  fields.forEach((field) => {
    ...
    if (field === 'images') {
      items[field] =
        dataByLanguagePostAndImage[language][slug] ||
        dataByLanguagePostAndImage[language]['news-512'] ||
        dataByLanguagePostAndImage['any-language']['any-article']
    }
    if (data[field]) {
      items[field] = data[field]
...
export async function getPostsNews(fields = []) {
  const postsByLanguage = readdir(join(postsNewsDirectory)).then((fileNames) => {
    return Promise.all(
      fileNames.map((name) => {
        const slug = name.replace(/\.md$/, '')
        return getPostNewsBySlug(slug, 'ru', fields)
      }),
    )
  })
