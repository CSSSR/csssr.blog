...
<Note isQuoteLike>
  **Подсказка** Часто мы забываем о том, какую большую роль может сыграть хороший логический прием.
</Note>
...
<Note isQuoteLike>
  **Подсказка** Не всегда есть возможность влиять на разные подсистемы проекта. Когда такая возможность есть, очень здорово посмотреть на решаемую задачу со всех сторон.
</Note>
...
<Note isQuoteLike>
  <p>**Подсказка** Читайте документацию и Stackoverflow внимательно. Невнимательность обязательно отнимет ваше время.</p>
</Note>
...
<Note isQuoteLike>
  **Внимание!** Be careful when using `select as` and `track by` in the same expression. © *<a href="https://angularjs.org" rel="nofollow noopener" target="_blank">angularjs.org</a>* В предыдущем блоке `ng-options` будет работать некорректно, подробнее читайте в документации по ng-options.
</Note>
...
<Note isQuoteLike>
  **Внимание!** Нужно быть осторожным с коллекциями, которые состоят из объектов. 1 === 1 всегда, но далеко не факт, что 2 объекта, у которых идентичны поля и их значения, будут равны. Помните, что объект — **ссылочный тип данных**.
</Note>
...
Надеюсь, что эта статья оказалась полезной и интересной. Смотрите работающие примеры кода из этой статьи <a href="https://github.com/gitVn/ng-option_example" rel="nofollow noopener" target="_blank"> в репозитории GitHub</a> и ждите новых записок от начинающего Angular-онавта! :)
