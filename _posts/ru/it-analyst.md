...
**В**озьмем среднестатистический IT проект, на котором есть несколько разработчиков, менеджер проектов и аналитик. Разработчики пишут код. Менеджер контролирует работы по проекту. А аналитик... что-то анализирует?
...
Аналитик занимается переводом с языка бизнеса на технический и наоборот. Проще говоря, это человек, превращающий «хотелку» в конкретную задачу. Так, если бизнес хочет поэкспериментировать с анализом моделей причинно-следственных связей, аналитик сформулирует это желание в функциональное требование, т.е. задачи на разработку таблиц для хранения элементов, методов редактирования и форм просмотра.

В целом процесс выглядит следующим образом. Аналитик собирает требования заказчика. Если это бизнес-требования, то они потребуют согласования через тернистый путь правок. После того, как требования собраны, аналитик переводит бизнес-требования в функциональные, т.е. набор конкретных изменений в системе, консультирует команду, отвечает за все изменения перед разработчиками и тестировщиками. После реализации он может подключаться к тестированию и последним этапам сдачи системы заказчику. Продуктовый аналитик должен следить за метриками продукта и анализировать последствия введения нового функционала. Наконец, аналитик помогает менеджеру проекта — расставляет приоритеты и управляет рисками.
...
Продуктовый аналитик занимается анализом использования продукта его пользователями. Он работает с пользовательскими метриками, выявляет причины различных явлений — оттока, возврата, эффекта от внедрения фич. А также придумывает А/Б тесты, воронки, ставит эксперименты и собирает предпосылки для формирования новых требований к системе, при этом не принимая участия в их разработке.
...
Пример: владелец интернет-магазина хочет увеличить количество заказов. Продуктовый аналитик выясняет, что покупатели часто бросают оформление покупки на полпути — чтобы сделать заказ, нужно заполнить множество полей в форме и подтвердить согласие с политикой конфиденциальности. Возможно, форма заказа должна быть упрощена, или стоит добавить покупку «в один клик», как у конкурентов. Аналитик проверяет гипотезы, проводит доработку и анализирует влияние нововведений на количество заказов в магазине.
...
Тестировщикам документация необходима для подготовки тест-кейсов и понимания бизнес-требований, помогая обеспечивать более качественный подход к тестированию.
...
А если используете, то не забывайте добавлять их в глоссарий.

#### Используйте ссылки на требования

В коде мы стараемся ссылаться на функции и объекты — функция перевода времени пишется один раз и повторно используется в остальных участках кода. То же самое и в документации: если требование зафиксировано в одном месте, то во всей остальной документации нужно дать на него ссылку. Это избавляет от необходимости вносить правки в каждую страницу при малейшем изменении требований.

#### Задавайте вопросы

Ответ на вопрос «зачем вам это нужно» может изменить весь подход к реализации требований. Например, заказчик считает, что ему нужно выгружать отчеты в Excel для ежедневного сравнения показателей. Выходит, что Excel — это не главное, ведь на самом деле заказчику нужен удобный инструмент для сверки метрик, который в реализации может оказаться проще и изящнее, чем обычная выгрузка отчетов в Excel.

### Как менеджерить изменения и правки

Документация не вечна. Изменения будут вноситься постоянно, поэтому вам заранее стоит наладить процессы управления изменениями на проекте. Самый простой вариант укладывается в четыре шага:

- зафиксировать изменения в реестре или в Jira, книге учета изменений, указав автора и дату правки;
- провести анализ изменений, назначить им приоритет;
- принять решение — правим, не правим, или правим, но позже;
- начинать работу по стандартным процессам: реализуем, тестируем, выкатываем.

Чаще всего изменения начинаются с четвертого пункта — это неправильно. Иногда всё ограничивается таской в Jira — это **совсем** неправильно.
...
Бывает, что колоссальное количество правок по уже реализованному продукту приходит от человека, который не был обозначен как главный контакт со стороны заказчика в части проработки требований. Скорее всего, вы пропустили еще одно заинтересованное лицо, мнение которого обязательно стоит учитывать при написании и согласовании требований. Поговорите с заказчиком: возможно, стоит подключать этого человека как можно раньше — например, на этапе проработки требований.

Реестр изменений позволяет контролировать поток правок и запросов на дополнительный функционал со стороны заказчика — он наглядно показывает, сколько времени (а значит, и денег) потребуется на реализацию всех идей.

Наконец, вы можете пропустить требования из-за отсутствия дара телепатии. Заказчик считает, что что-то должно быть по умолчанию, поскольку это есть у всех, и не указывает наличие этого элемента как требование. Решить эту проблему поможет шаблон требований и, конечно же, опыт.
